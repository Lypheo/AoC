...
def solve_a(inp=input_data):
    ...
    valid = {(x,y,z) for x in range(-50, 51) for y in range(-50, 51) for z in range(-50, 51)}
    cube = set()
    for i in ins:
        on = i[-1] == "on"
        switch = set()
        for x,y,z in itertools.product(range(i[0][0], i[0][1]+1), range(i[1][0], i[1][1]+1), range(i[2][0], i[2][1]+1)):
            if (x,y,z) not in valid:
                switch = set()
                break
            switch.add((x,y,z))
        # print(switch)
...
def partition(c1, c2):
    ...
    def blocks(nums):
        if not nums:
            return nums
        contiguous_blocks = [[nums[0], nums[-1]]]
        for k in range(1, len(nums)):
            if nums[k-1] != nums[k] - 1:
                contiguous_blocks[-1][1] = nums[k-1]
                contiguous_blocks.append([nums[k], nums[-1]])
        return contiguous_blocks

    ps = set()
    dim = len(c1)
    s, us = dd(list), dd(list)
    for i in range(dim):
        unshared = set(range(c1[i][0], c1[i][1]+1)) - set(range(c2[i][0], c2[i][1]+1))
        shared = set(range(c1[i][0], c1[i][1]+1)) & set(range(c2[i][0], c2[i][1]+1))
        if not shared:
            return {c1}
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507""" : [590784, 2758514936282235]
}
