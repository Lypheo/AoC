...
def solve_a(inp=input_data):
    ...
    def get_patterns(r):
        ...
        for rule in r.split():
            rr = rules[int(rule)]
            p.append([rr.strip("\"")] if "\"" in rr else get_patterns(rr))

        return ["".join(x) for x in itertools.product(*p)]

    patterns = get_patterns(rules[0])
    return sum(m in patterns for m in msgs)
...
def solve_b(inp=input_data):
    ...
    lens = {i: len(get_patterns(rules[i])[0]) for i in [8, 42, 31]}
    ...
    def match(s: str):
        ...
        if s[:lens[8]] not in get_patterns(rules[8]):
            ...
            return match11(s[lens[8]:]) or match(s[lens[8]:])

    return sum(match(m) for m in msgs)
